...
//        file_put_contents(__DIR__."/verify.txt",$pubKey);
//        file_put_contents(__DIR__."/verify.txt",$data.PHP_EOL,FILE_APPEND);
        //调用openssl内置方法验签，返回bool值
        $result = (bool)openssl_verify($data, base64_decode($sign), $res, OPENSSL_ALGO_MD5);
//        file_put_contents(__DIR__."/verify.txt","verify result:".$result.PHP_EOL,FILE_APPEND);
        //释放资源
        openssl_free_key($res);
     ...
     */
    function parseResp($content, $keyPrivate, &$respJson) {
        ...
        if(array_key_exists("sign", $resp)) {
            //校验签名
            $keyPrivate = $this->formatPubKey($keyPrivate);
            return $this->verify($resp["transdata"], $resp["sign"], $keyPrivate);
